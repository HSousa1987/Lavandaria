-- ==============================================
-- Lavandaria PostgreSQL Database Schema v2.0 (SIMPLIFIED)
-- ==============================================
-- SINGLE SOURCE OF TRUTH - Simplified for Portugal-only operations
-- Date: 2025-11-09
-- Purpose: Streamlined dual-business system with property-based workflow
-- ==============================================
...
-- ==============================================
-- ROLE TYPES (Lookup table for user roles)
-- ==============================================
...
-- Insert role types
INSERT INTO role_types (role_name, description) VALUES
('master', 'System owner with full access'),
('admin', 'Manager with admin privileges'),
('worker', 'Field worker for jobs and orders')
ON CONFLICT (role_name) DO NOTHING;

-- ==============================================
-- PROPERTY TYPES (Lookup table for property categories)
-- ==============================================
...
CREATE INDEX idx_clients_is_enterprise ON clients(is_enterprise);

-- ==============================================
-- PROPERTIES (Client addresses for services)
-- ==============================================
-- Workflow: Client → Properties → Cleaning Jobs
-- A client can have multiple properties (10+ addresses)
-- When creating cleaning job, select client first, then property
...
CREATE INDEX idx_properties_is_primary ON properties(is_primary);

-- ==============================================
-- CLEANING JOBS (Airbnb & Houses)
-- ==============================================
-- Uses property_id instead of direct address
-- Select client → shows only that client's properties
...
CREATE INDEX idx_cleaning_jobs_property ON cleaning_jobs(property_id);
...
CREATE INDEX idx_photos_worker ON cleaning_job_photos(worker_id);
...
CREATE INDEX idx_laundry_services_type ON laundry_services(service_type);
CREATE INDEX idx_laundry_services_active ON laundry_services(is_active);
...
-- ==============================================
-- Single payments table for both services
-- service_type determines if cleaning or laundry
-- Tax is flexible per receipt (master/admin can change)
CREATE TABLE payments (
    id SERIAL PRIMARY KEY,
    client_id INTEGER NOT NULL REFERENCES clients(id) ON DELETE CASCADE,

    -- Service discrimination (one will be NULL)
    ...
    -- Client always pays same amount, tax % can vary for accounting
    tax_percentage DECIMAL(5,2) NOT NULL DEFAULT 23.00, -- IVA 23% (Portugal standard)
    tax_amount DECIMAL(10,2) NOT NULL, -- Auto-calculated
    amount_before_tax DECIMAL(10,2) NOT NULL, -- Auto-calculated
...
CREATE INDEX idx_payments_cleaning_job ON payments(cleaning_job_id);
CREATE INDEX idx_payments_laundry_order ON payments(laundry_order_id);
...
CREATE TABLE tickets (
    ...
    service_type VARCHAR(20) CHECK (service_type IN ('laundry', 'cleaning', 'general')),
    cleaning_job_id INTEGER REFERENCES cleaning_jobs(id) ON DELETE SET NULL,
    laundry_order_id INTEGER REFERENCES laundry_orders_new(id) ON DELETE SET NULL,
...
-- ==============================================
-- JOB NOTIFICATIONS (for client alerts)
-- ==============================================
...
-- Auto-calculate payment tax amounts
CREATE OR REPLACE FUNCTION calculate_payment_tax()
RETURNS TRIGGER AS $$
BEGIN
    -- Calculate amount before tax and tax amount based on tax percentage
...
('master', '$2b$10$iyoWMyQRwUrseBPEUOvu6.9xMRJ4d6RCjIbfv/OpollcbeMQWiU.e', 1, 'Master Admin', 'master@lavandaria.com'),
('admin', '$2b$10$AC7dVLo.KeW3YgIeveZ9C.Dy/qmDxQhoDtvf0Q2vW2k/EZyy8uNEy', 2, 'Administrator', 'admin@lavandaria.com'),
('worker1', '$2b$10$5TBBvMz.csBeXlp/isBgnuMi8xlMGywuot8VUxN5QaP5Qz7ELYZJW', 3, 'Maria Silva', NULL)
ON CONFLICT (username) DO NOTHING;

UPDATE users SET created_by = 1 WHERE username IN ('admin', 'worker1');

-- Insert sample client
INSERT INTO clients (phone, password, name, email, notes) VALUES
('911111111', '$2b$10$XYjLzqW8K9H.eN6D9h3.8OZN.q7K6r3zv3fV3d7N.H8K9XYjLzqW8K', 'João Santos', 'joao@example.com', 'Sample client for testing')
ON CONFLICT (phone) DO NOTHING;

-- Insert sample properties for client
INSERT INTO properties (client_id, property_name, address_line1, city, postal_code, property_type_id, is_primary) VALUES
(1, 'Casa Principal', 'Rua das Flores, 123', 'Lisboa', '1100-123', 1, TRUE),
(1, 'Apartamento Airbnb', 'Av. da Liberdade, 456', 'Lisboa', '1200-456', 2, FALSE)
...
-- ==============================================
COMMENT ON TABLE users IS 'Staff users: master, admin, worker (role via role_types FK)';
COMMENT ON TABLE clients IS 'Customer accounts (Portugal-only, simplified addressing)';
COMMENT ON TABLE properties IS 'Client properties for cleaning services (one client, many properties)';
COMMENT ON TABLE property_types IS 'Lookup table for property categories (casa, apartamento, quinta, etc.)';
COMMENT ON TABLE role_types IS 'Lookup table for user roles (master, admin, worker)';
COMMENT ON TABLE cleaning_jobs IS 'Cleaning jobs linked to properties (select client → shows their properties)';
...
COMMENT ON TABLE payments IS 'Unified payments table for both cleaning and laundry services with flexible tax handling';
COMMENT ON TABLE job_notifications IS 'Notification system for client alerts';
COMMENT ON TABLE tickets IS 'Worker issue reporting system';

COMMENT ON COLUMN payments.tax_percentage IS 'Tax percentage for this receipt (default 23% IVA, master/admin can change per receipt)';
COMMENT ON COLUMN payments.tax_amount IS 'Auto-calculated tax amount (updated by trigger)';
COMMENT ON COLUMN payments.amount_before_tax IS 'Auto-calculated amount before tax (updated by trigger)';
COMMENT ON COLUMN payments.service_type IS 'Discriminates between cleaning and laundry payments';
